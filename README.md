...
```text
rehab/
├── rehab_rkz/               # Modul Utama Aplikasi (Root Aplikasi PHP)
...
2. **Setup Server**
   ...
   - Pindahkan repositori ini ke dalam folder `htdocs` atau `www`.
3. **Setup Database**
   - Buka `rehab_rkz/config/database.php` dan sesuaikan *Username* serta *Password* database MySQL Anda.
   - Buka URL `http://localhost/rehab_medis/rehab_rkz/setup_db.php` melalui browser Anda satu kali saja untuk membuat database dan tabel yang dibutuhkan secara otomatis.
4. **Jalankan Aplikasi**
   - Akses aplikasi pada URL: `http://localhost/rehab_medis/rehab_rkz/`
   - Gunakan kredensial (NIP/Username dan Password) yang terdaftar di tabel `hrd.datadasar` untuk login.
